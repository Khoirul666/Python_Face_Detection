            ...
            for result in results:
                for box in result.boxes:
                    ...
                    if(f'{model.names[cls]}'=='helm'):
                        await Color(resized_frame,(0, 255, 0),1,f'{model.names[cls]} {conf:.2f}',x1,x2,y1,y2)

                    # Gambar class kepala dengan kotak merah dan beri warna merah dan tebal 1px serta label
                    if(f'{model.names[cls]}'=='kepala'):
                        await Color(resized_frame,(255, 0, 255),1,f'{model.names[cls]} {conf:.2f}',x1,x2,y1,y2)

                    # Gambar class pengendara dengan kotak biru dan beri warna biru dan tebal 1px serta label
                    if(f'{model.names[cls]}'=='pengendara'):
                        await Color(resized_frame,(255, 0, 0),1,f'{model.names[cls]} {conf:.2f}',x1,x2,y1,y2)
